...
function start_shadowbox() {
  ...
  local data_dir="$(pwd)/shadowbox-data/"
  check_and_remove_item "${data_dir}"
  check_and_remove_item "${compose_file}"
...
services:
    ...
    watchtower:
        ...
        command: --cleanup --label-enable --tlsverify --interval ${WATCHTOWER_REFRESH_SECONDS:-3600}
"
...
function start_watchtower() {
  echo "Watchtower will be managed by Docker Compose."
}

function verify_docker_compose_installed() {
  if ! command_exists docker-compose; then
    echo "Docker Compose is not installed. Installing now..."
    # The installation command varies by system; adjust as necessary.
...
install_shadowbox() {
  ...
  run_step "Verifying that Docker Compose is installed" verify_docker_compose_installed
...
ACCESS_MANAGER=$(echo -e "\033[1;32m{\"apiUrl\":\"$(get_field_value apiUrl)\",\"certSha256\":\"$(get_field_value certSha256)\"}\033[0m")


${FIREWALL_STATUS}
END_OF_SERVER_OUTPUT

# echo -e "${ACCESS_CONFIG}
# ${FIREWALL_STATUS}
# " > ${SHADOWBOX_DIR}/access_info.txt
